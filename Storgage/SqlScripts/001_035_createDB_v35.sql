...
EmailVerificationStatusID uniqueidentifier not null
...
unique
 index idx_User_Email_unique_incl
    on dbo.[User](email)
	   include (Id, Password, Phone/*, IsPhoneVerified*/, Lastname, Firstname, PaymentSystemSecret, AvgRate, PhoneVerificationStatusID, AvatarLink, CountRating, FacebookID, EmailVerificationStatusID)
 ...
 */
 ...
 IsDeleted bit not null
  ...
  --I don't think that we should remove AdSpace entity and use its attributes here
...
*/
create 
 index idx_Message_UserId_SentDate_incl
    on dbo.Message(UserId, SentDate)
...
-- drop 
 index idx_Message_ChatId_incl
    on dbo.Message(ChatId)
       include (Id, /*Rate,*/ ReceivedDate, SentDate, Message, /*RentSince, RentTill, StatusId,*/ UserId, DeliveredStatusId/*, RecepientId*/)
go
 ...
 select --This logic will work for two users ONLY
  ...
  inner
   join cteChangedRows chR
	...
	and chR.SenderID <> msg.UserId --we exclude senders
